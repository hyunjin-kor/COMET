...
### H3. 표지 후보 (2026-09-10, 커밋하지 않음)

사용자 요청('GPT랑 제미나이로 조합해서 만들자, PPT에서 재조합해서 새로운 이미지로, 전체 저널 규정에 맞춰서')에 따라 표지 후보를 만들었다. 먼저 규정을 정리해 사용자에게 알렸다: **PPT에서 재조합해도 AI 이미지는 AI 이미지다.** ACS 규정은 회피가 아니라 공개를 요구하며, 공개할 자리가 없는 곳은 TOC 그래픽뿐이다. 따라서 Fig. 1(이미 공개함)과 표지(캡션에 공개)는 AI 사용이 허용되고, TOC는 계속 직접 그린 것만 쓰며, Fig. 2·3은 데이터 도판이라 애초에 대상이 아니다.

- 같은 저자 작성 명세로 ChatGPT(PSID_KHU 계정, 1254×1254)와 Gemini(사용자 계정, 1024×1024)에서 각각 1장 생성해 `_local/note-figures/ai-renders/`에 원본 그대로 보관.
- `_local/note-figures/compose_cover_art.py`가 세로 8.5×11 in, 300 dpi(2550×3300) 캔버스에 두 장을 합성한다. 각 층의 자체 배경값을 뺀 뒤 screen 합성해 경계가 남지 않게 했고, Gemini 렌더에서는 입자 꼬리 부분만 잘라 좌우 반전·타원 페이드 후 회전(회전 전에 페이드해야 직선 자국이 남지 않는다)해 오른쪽 위로 이어 붙였다. 왼쪽 위는 masthead 자리로 비웠다. Gemini 렌더 우하단의 네 갈래 표식은 크롭에서 제외했다.
- `_local/note-figures/build_cover_pptx.py`가 같은 좌표로 두 층을 검은 슬라이드에 올린 PPT(`cover_composition.pptx`)와 원본 2장·합성본을 함께 담아 손으로 옮길 수 있게 한다.
- `_local/note-figures/cover_art.provenance.md`에 프롬프트 전문, 생성 도구·날짜·계정, 저자 합성 방법, ACS 표지 규정과 제안 캡션 문구를 적었다.
- 확인 못 함: OpenAI·Google 이용약관의 출력물 권리(자동 요청 차단), Gemini 파일의 비가시 워터마크 유무, 저널이 요구하는 표지 판형·해상도. 8.5×11 in 300 dpi는 인용된 요건이 아니라 작업 가정이다. 표지는 보통 저널이 요청할 때 내는 것이므로 제출 여부는 사람이 정한다.
