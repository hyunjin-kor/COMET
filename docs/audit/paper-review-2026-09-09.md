...
### H59 — 2026-09-21: TOC 그래픽 제목 색(머리글자 검정, 소문자 회색), 루테늄·세 금속 시안 검토

- 저자 요청으로 TOC 제목의 머리글자 C·O·M·E·T를 검정 굵은 글자로, 나머지 소문자를 회색으로 바꿨다(이전: 머리글자 청록).
- 저자 질문에 따라 미리보기 두 개를 만들어 비교했고, 둘 다 저장소에는 반영하지 않았다.
  - 암모니아 분해 그래프에 Ru/MgO를 끊긴 축으로 넣은 시안: Ru가 약 300배 비싸고 최근 오름을 보여 주지만 Co·Ni 교차가 덜 보인다. 월별 자료에서 Ru 가격은 250 → 1,708 USD/oz로 6.8배 움직였다(Co 3.8배, Ni 2.9배). Ru/MgO 원가는 442–2,983 USD/kg이며 한 번도 최저 원가가 아니었다.
  - 생산 규모 그래프에 Ru·Co·Ni 세 금속을 넣은 시안: Co/Al₂O₃(20 wt%) 곡선은 동결 자료에 없어 계산기 API로 임시 계산했다(60.39 → 14.42 USD/kg, 4.2배; Ni 9.1배, Ru 1.5배). 세 자릿수에 걸친 로그축에서는 기울기가 밋밋해 "규모가 클수록 단가가 낮다"는 인상이 약해진다.
  - Co를 본문 what-if 분석(Figure 4(a,b), Table S8)에 추가할지는 저자 결정으로 남겼다. 추가하려면 what-if 연구를 다시 동결해야 한다.
- 본문·수치·분량은 H57과 같다(4,997/5,000). 검사: 노트·SI `--check`, ruff, 노트·질량 단위 테스트(18 passed).
- Word v59(`_local/docx/rebuild_v59.py`): EN 15쪽, KO 20쪽, SI 22쪽, 서식 검사 통과. 패키지는 `_local/submission-2026-09-21-v59/`다. 외부 업로드·푸시 없음.

### H60 — 2026-09-22: 담지량 공개, 제조법에 따른 원가 역전, 공개 가격 근거 표

- 저자 질문(마진·G&A·SARD가 촉매 비교를 희석하는가, 함량이 현실적인가, 제조법에 따라 경제성이 역전되는 사례를 보일 수 있는가)에 따라 원고를 확장했다.
  - 마진·G&A·SARD는 곱하는 항이라 같은 생산 규모에서는 비율·순위를 바꾸지 않는다(Ni/Al₂O₃ 제조법 5종: 재료비+가공비 차이 +4.2/+12.2/+4.2/+1.7% = 판매 단가 차이). 희석은 라이브러리의 덧셈 항인 경로별 추가 비용(Co 대 Ni +14.4% → +12.9%)과 후보마다 다른 생산 규모(17/30 반응군)에서 생긴다. 공통 규모 재동결과 추가 비용 처리는 저자 결정으로 남겼다.
  - 담지량은 후보마다 라이브러리 값이다(1 wt% 통일 아님). 암모니아 분해: Co 5 wt%(1차 문헌 인용), Ru 3 wt%(3.1 wt% 근사), Ni 12 wt%·Ni 15 wt%(공학적 가정). 116개 후보 전체의 활성 금속(또는 벌크 활성상)과 생산 규모를 SI Table S7에 두 열로 추가했고, 본문 라이브러리 문단과 암모니아 분해 문장·Figure 4(d) 범례에 담지량을 넣었다.
- `scripts/note_whatif_study.py`에 `route_assignment` 절을 추가했다: 담지 금속 후보 70개(반응군 20개; `pdcu-saa`는 계산기 요청이 받지 않는 host_metal 역할이라 제외)를 라이브러리 조성 그대로 기준 가격에서 제조법 5종·18,143.7 kg으로 계산한다. 재료비는 1차 실행의 동결값과 일치해야 하고(가드), 가공비는 조성과 무관해야 하며(가드), 같은 제조법을 모든 후보에 적용하면 순서가 바뀌지 않아야 한다(가드). 기존 절은 바이트 단위로 동일하다.
  - 결과: 제조법 5종은 모든 후보의 판매 단가를 1.26 USD/kg 범위로 움직이고, 재료비 차이가 그보다 작은 5개 반응군(암모니아 분해, CO₂ 메탄화, 메탄 건식 개질, 수첨탈산소, 메탄 수증기 개질)에서 최저 원가 후보가 제조법 배정에 따라 바뀔 수 있다. 암모니아 분해: Ni/γ-Al₂O₃는 재료비가 0.86 USD/kg 낮지만 침착–침전으로 만들면 초기습윤 함침의 Co/MgO–La₂O₃보다 0.09 USD/kg 비싸다(25개 조합 중 1개).
- 본문: 「Preparation-method crossovers」 소절 신설(제조법이 순서를 뒤집는 조건과 사례, 20개 반응군 중 5개, 활성 동일 가정 없음), 결론에 반영, 한계에 Table S12 참조. 분량을 맞추려 문장 단위로 다듬었다(서론 예시 구절, 실험실 배치 마지막 문장, 감사의 글 한 문장 등 삭제). 4,999/5,000(초록 136 + 본문 2,163 + 도판 2,400 + TOC 300). Word 통계도 136·2,163으로 같다.
- Figure 4: (d) 범례에 담지량(5 wt% Co/MgO–La₂O₃, 12 wt% Ni/γ-Al₂O₃), (e) 폭 축소·후보명 두 줄 배치, (f) 신설(Co 판매 단가 − Ni 판매 단가의 5×5 제조법 조합 행렬, 색은 더 저렴한 후보; IW·WI·DP·CP·SG 약어는 캡션에 정의). 음수는 스크립트 관례대로 하이픈(Malgun Gothic에 U+2212가 없음).
- SI: Table S7에 「활성 금속 또는 활성상(wt%)」·「생산 규모(kg)」 열 추가; Table S9(제조법 5종에서의 원가 순서, 20개 반응군) 신설; 옛 S9·S10은 S10·S11; S7절 「External price evidence」 신설 및 Table S12(2026-09-07 선별한 공개 가격 사례 10건, 일치 차원)로 본문의 "산업 가격 대비 정확도 미확립" 근거를 제시; S7·S8절은 S8·S9. SI 자체 검사에 동결 실행과 라이브러리 후보 일치, 완전 일치 사례 없음 가드 추가.
- 한글 원고(`_local/docx/application-note-2026-09-09_ko.md`)에 같은 내용을 반영했다(저자 블록 불변).
- 검사: 노트·SI·what-if `--check`, ruff, 노트·질량 단위 테스트 18 passed(변환 25건·per_lb 14건, Table S7 열 검사 갱신). Word v60(`_local/docx/rebuild_v60.py`): 도판 위치를 v60 쪽매김 탐색으로 다시 정했고(EN: Fig. 2 라이브러리 문단 뒤, Fig. 3·4 what-if 문단 뒤; KO: Fig. 2 Step Method 문단 뒤, Fig. 4 제조법 역전 문단 뒤) 재탐색으로 안정성을 확인했다. EN 15쪽, KO 20쪽, SI 27쪽, 서식 검사 통과(표 12개). 패키지는 `_local/submission-2026-09-21-v60/`. 외부 업로드·푸시 없음.

### H61 — 2026-09-22: 화학공학 어휘로 전수 교체 (소프트웨어·전산 어투 제거)

- 저자 지적("operating input은 안 쓰이고 operating condition(운전 조건)을 쓴다. 화학공학 박사가 쓰는 어휘로, 전수 검토")에 따라 본문·도판·SI·문헌 검토 문서·한글 원고의 어휘를 바꿨다. 수치·분량 논리는 그대로다(4,999/5,000; Word 통계 136·2,163 동일).
- 주요 교체(영문): operating/declared inputs → operating conditions(stated conditions); inputs(일반) → conditions·variables·data; provenance → preparation procedures / source quality; literature catalyst architecture → literature formulation; engineering baselines → an engineering baseline; flagging → noting; specimen-specific → sample-specific; locator → source section; editable → modifiable; flags → identifies; silently/audited → unstated/checked; implementation(검증 문장) → calculation; what-if → parametric analyses(소절·Fig. 4 캡션·SI Table S8); balanced profile → balanced weighting; leader → first-ranked candidate; live basis/quotations → current quotations; cost leaders → least expensive candidates; dry output → dry product mass; attended labor → operator labor; equipment occupancy → equipment time; step rates → throughput of each step; treatment duration → treatment time; records → documents(동사); audit → review; default → assuming a value.
- SI: S2 「Stated batch conditions」, S5 「Price basis and screening results」, S6 「Literature preparation procedures and missing conditions」, S7 「Published catalyst prices」; frozen → archived, replay → recalculation, curated/variant → documented/source-specific preparation, flagged → noted, catalog → library, clamping → truncation, seeded → fixed random seed, legacy fields → original values, isolated database → separate test database; Table S10·S11·S12 제목과 머리글 갱신. `scripts/paper_labels.py`의 상태 라벨 3종을 바꿨고(Figure S7·SI Table S11·문헌 검토 문서에 공통), `docs/paper/manufacturing-literature-2026-09-14.md`를 다시 생성했다.
- 도판: Figure S2 범례 Lower/Upper value, Figure S3 x축 Sampled dry product mass, Figure S7 상태 라벨, Figure 3(a) 라벨 Dry output → Dry product(덱 relabel). 한글 도판 라벨도 같이 바꿨다.
- 한글 원고: 운전 입력 → 운전 조건, 입력 → 조건·변수·자료, what-if → 매개변수 분석, 균형 프로필 → 균형 가중치, 구현 검증 → 계산 검증, 촉매 구조 → 조성, 위치 정보 → 출처의 해당 절 등 34곳.
