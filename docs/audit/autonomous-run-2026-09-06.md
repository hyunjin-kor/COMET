...
실적 기록 기능이 생겼지만 **산업 전체 제조 원가에 대한 독립 정확도 검증은 여전히 없습니다**. 사용자 확인은 독립 검증이 아니며, 통화·가격 기준월·조성·등급·규모·생산 속도·제조 단계·비용 범위가 맞지 않는 기록은 보관만 합니다. 전극과 실제 투입량을 대조하지 못한 전구체·소모품 계산은 현재 오차 평균에서 제외합니다. 적격 사례가 없을 때 MAPE는 `null`입니다.
...
## 제조 화면 후속 수정 — 2026-09-07

사용자가 바탕화면 설치본에서 확인한 재선택 오류와 화면 크기 변화를 수정했습니다. 이번 M01–M03은 **완료 3개·보류 0개·부분 완료 0개**, 구현 커밋은 `aba3b3f`입니다. FCC의 반복 단계를 해제·재선택했을 때 총 단계가 25 → 17로 줄던 오류를 25 → 16 → 25로 복원했고, 선택된 제조법·저장된 편집·규모 변경 중 편집을 보존합니다. 검색 목록과 상세 영역, 고정 크기의 체크 행으로 제조 화면을 바꾸고 설치본을 갱신했습니다.

전체 pytest **780 passed/448.08s**, 계산기 규칙 17개·추정 범위 규칙 6개, Ruff·프론트 lint/build·i18n, 실제 Chrome 6개 흐름, Windows build **203.033s**와 smoke **17.643s**가 통과했습니다. Table 6.2는 Pt/C 27.37, Ni/Al₂O₃ 19.2206(−6.65%), FCC 각주 b 2.4380(+1.16%) USD/lb로 유지됩니다. 설치 파일 해시 일치, DB 무결성 `ok`, 버전 1.4.0·주 창 1개도 확인했습니다.

[작업 표·계산 영향·가정·설치 확인](manufacturing-selection-2026-09-07.md), [명령과 해시](manufacturing-checks-2026-09-07.json), [브라우저 결과](manufacturing-ui-2026-09-07.json)에 근거를 모았습니다. API 처리 성능은 이번에 다시 측정하지 않았으며 새 자료나 미산정 단가를 추가하지 않았습니다. 기존 D03·V01의 부분 완료 상태와 사람의 태그·릴리스·Zenodo·투고 확인 사항은 그대로입니다. 같은 PR112를 유지하고 master 머지는 하지 않았습니다. 최종 감사 기록을 포함한 head의 CI 링크는 PR 본문에 남깁니다.


## 2026-09-07 후속: 과학 표기의 위·아래첨자

앱과 CSV의 화학식·이온 전하·동위원소·단위 지수를 통일하고 바탕화면 설치본에 반영했습니다. S01–S03은 **완료 3개·보류 0개·부분 완료 0개**, 구현 커밋은 `a7f2d56`입니다. 원본 데이터·JSON·계산 요청값·가격·계산식은 유지하며 첨자 검색과 분자식 표시를 보완했습니다. 오래된 이름 5개의 Al23/Al203/A1203는 원출처를 확인하지 못해 추정 수정하지 않았습니다.

전체 pytest **780 passed/444.01s**(명령 전체 448.599s), 표기 6개·계산기 17개·추정 범위 6개, Ruff·lint/build·i18n이 통과했습니다. Chrome 11개 화면과 제조 회귀 6개 흐름에서 오류 0건입니다. Windows build **198.068s**, smoke **17.604s**, 설치 종료 0, 패키지 해시 일치, DB 무결성 ok와 설치본 1.4.0·주 창 1개를 확인했습니다.

Table 6.2는 Pt/C 27.37, Ni/Al₂O₃ 19.2206(−6.65%), FCC 각주 b 2.4380(+1.16%) USD/lb로 유지됩니다. 이번 표기 변경에 따른 계산 결과 차이는 없으며 실행 성능은 다시 측정하지 않았습니다. [작업 표·가정·예외·화면·설치](scientific-notation-2026-09-07.md), [명령·시간·해시](scientific-checks-2026-09-07.json)에 근거를 기록했습니다. 최종 head CI는 PR112 본문에 남깁니다. 이전 D03·V01, 태그·실제 릴리스·Zenodo·저널/투고의 사람 확인 사항은 그대로입니다. master 머지·태그·릴리스·외부 게시·유료 자료 사용은 하지 않았습니다.
