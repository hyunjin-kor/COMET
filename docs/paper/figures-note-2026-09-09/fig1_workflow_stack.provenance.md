...
The current `fig1_workflow_stack.png` and SVG are generated by `scripts/draw_application_note_figures.py`. The diagram uses the layout of an earlier AI-assisted illustration as a reference. This note preserves both stages of its creation so that the caption and AI-use statement can be checked.
...
| `fig1_workflow_stack.png`, `fig1_workflow_stack.svg` | Current diagram, rendered from `figure1_workflow()` in `scripts/draw_application_note_figures.py` |
| `fig1_workflow_stack.ko.png`, `fig1_workflow_stack.ko.svg` | Korean diagram rendered by the same function with `--lang ko` |
...
The manuscript underwent a complete language review after the author identified ambiguous, abbreviated labels. That redraw retained the five-stage layout and associated records but removed pictograms and explanatory bullet fragments. Details of the data, costing procedures, and sensitivity analyses appear in the text and caption. In particular, the candidate-removal analysis is no longer labelled a leave-one-out test, which could be confused with cross-validation. The raw AI images and original prompts below are unchanged. No AI image generator was used for this redraw, and the current figure contains no raster pixels from the earlier illustration.

The author subsequently requested an icon for each stage in the v27 document review. The current version adds original geometric icons drawn by `_workflow_icon()`: a database, calendar, calculator, stacked cost bars, and ranking podium. The stage labels share a left alignment beside the icons. These are vector primitives in the same generator, with no imported icon assets or new image-generation call. Explanatory prose remains in the caption and text.

Regenerate the current English and Korean diagrams with:
...
The caption and acknowledgments disclose the earlier use of ChatGPT image generation as a layout reference. The historical rights review below is retained; the redraw does not constitute a new legal determination.
