...
The current `fig1_workflow_stack.png` and SVG are generated by `scripts/draw_application_note_figures.py`. The diagram reuses five stage icons from an earlier AI-assisted illustration, with labels and layout drawn programmatically. This note preserves its creation history so that the caption and AI-use statement can be checked.
...
The current version extracts five regions of the unchanged English raw image in `_workflow_icon()`: stacked price sheets, a calendar, a flask with a price tag, a cost ledger, and a balance scale. Crop coordinates are fixed in the generator. Only the pale background connected to each crop border is removed, then composited onto the stage fill before resampling to preserve the outlines. Icon proportions are preserved at a common height of 11.5 mm, with an aligned text column. The same icons are used in both languages. The SVG therefore combines embedded raster icons with editable vector text, boxes, and connectors; it is not an entirely vector illustration. No new image-generation call was made. Explanatory prose remains in the caption and text.
