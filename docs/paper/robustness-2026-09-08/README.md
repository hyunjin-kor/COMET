...
## Ledger note (2026-09-09)

In the frozen `decision_robustness.json`, `families[*].candidate_removal[*].after[*].scores.total` carries the pre-removal composite forward: the original `removal_audit` recomputed economics scores and ranked with explicit weights, but did not rewrite the ledger's `total` field. Winner flags, `fixed_scale_winner`, `renormalized_winner`, every summary value, the CSV and both figures are unaffected. 62 of the 250 after-ledger entries differ from the composite recomputed from their four score components with the balanced weights (for example, `co-mgo-la2o3` after removing `ru-mgo-premium` shows 89.3 where the recomputed value is 77.4). `scripts/run_decision_robustness.py` now applies `_apply_total_scores` after re-normalization. A replay with the current script reproduced this package byte-for-byte for `candidate_robustness.csv`, `decision_robustness.png` and `decision_robustness.svg`; the JSON differed only in those 62 leaves. The frozen file is retained unchanged so that its recorded provenance hashes stay valid; the [methods supplement](../methods-2026-09-09/methods_study.json) recomputes post-removal totals explicitly. Evidence: [independent review log](../../audit/paper-review-2026-09-09.md).
