...
- **Paper dataset**: `scripts/run_all_families.py --out <json>` evaluates all 30 families × 116 candidates offline, freezes the price basis it used, adds a performance-weight-zero ranking and a 286-point weight-simplex rank-stability sweep. Rerun it whenever prices, factors or benchmarks change so the reported numbers stay reproducible.
