...
- **Cobalt, molybdenum and tungsten price history**: the break-even sweep shows the nearest cost flips are Ni-vs-Co contests (ammonia cracking at Co −8%, dry reforming at Co −17%). Cobalt is covered since 2026-09-03: `fetch_price_history.py` freezes the IMF Primary Commodity Price System series (indicator `PCOBA`, monthly average, quoted per tonne despite the codelist's "per pound" label). Its 2025 average is $15.8/lb against the USGS U.S. spot cathode anchor of $21.0/lb the app uses as today's price, so move the app's cobalt basis to the same series before quoting cobalt month counts. Molybdenum and tungsten still have no free observed series: IMF PCPS does not carry them, and Westmetall/LME, Yahoo Finance and FRED were checked on 2026-09-03. Candidates are the USGS Mineral Industry Surveys (monthly PDF) or UN Comtrade unit values (API key required).
