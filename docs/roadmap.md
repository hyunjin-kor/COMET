...
- **Paper dataset**: `scripts/run_all_families.py --out <json>` evaluates all 30 families × 116 candidates offline, freezes the price basis it used, adds a performance-weight-zero ranking and a 286-point weight-simplex rank-stability sweep. `scripts/active_metal_breakeven.py` then finds, per family, the metal price at which the cost-only and composite winners flip (`backend/core/breakeven.py`) and counts the months of frozen history on each side. Rerun both whenever prices, factors or benchmarks change so the reported numbers stay reproducible. Both, and `price_volatility_screen.py`, accept `--price-basis <json>` to re-cost from a committed basis instead of the local price table.
- **Cobalt, molybdenum and tungsten price history**: the break-even sweep shows the nearest cost flips are Ni-vs-Co contests (ammonia cracking at Co −8%, dry reforming at Co −17%). Cobalt and molybdenum are covered since 2026-09-03: `fetch_price_history.py` freezes the IMF Primary Commodity Price System series (`PCOBA` and `PLMMODY`, monthly averages, quoted per tonne; the cobalt codelist entry says "per pound" but the values are per tonne). Two basis checks are open before cobalt or molybdenum month counts are quoted. Cobalt: the IMF 2025 average is $15.8/lb against the USGS U.S. spot cathode anchor of $21.0/lb the app uses as today's price. Molybdenum: the IMF 2025 average is $51.9/kg, the same figure USGS reports as "$51/kg" for molybdic oxide, which Platts quotes per kilogram of contained Mo; the app's anchor of $34.71/lb divided that figure by the oxide's Mo fraction a second time, so it is likely 1.5x high and should read about $23/lb. Tungsten still has no free observed series (IMF PCPS, Westmetall/LME, Yahoo Finance and FRED checked on 2026-09-03); candidates are the USGS Mineral Industry Surveys (monthly PDF) or UN Comtrade unit values (API key required).
