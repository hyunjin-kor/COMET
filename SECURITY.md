...
If you find a security issue in CatPrice — for example a vulnerability in
...
**https://github.com/hyunjin-kor/CatPrice/security/advisories/new**
...
- A reproduction path: minimum input, observed behaviour, expected
  behaviour.
- Any relevant logs from `%APPDATA%\CatPrice\catprice-launcher.log`
...
CatPrice is a local desktop tool. The following are not in scope for
the security policy:
...
- Issues that require an attacker to already have local user-level
  access to the machine running CatPrice.
