...
export default function ManufacturingProtocolSummary({ report, compact = false }: { report: ManufacturingReport; compact?: boolean }) {
  ...
  return <section className="mt-4 rounded-xl border border-teal-200 bg-white p-4">
    ...
    {!!report.intermediate_batches?.length && <div className="mt-4 overflow-x-auto"><p className="text-sm font-medium">{l('Intermediate cost allocation', '중간 배치 비용 배분')}</p>
      <p className="mt-2 text-xs leading-6 text-slate-600">{l('Mass allocation retains unused cost with recoverable inventory; whole-batch charging assigns all expenditure to the receiving batch. Each row lists only its own operations and purchases, avoiding overlap with upstream rows. Final allocated fractions include all subsequent transfers. Overhead and margin are excluded.', '질량 배분은 미사용 비용을 회수 가능한 재고에 남기고, 전체 배치 방식은 전액을 투입 대상 배치에 반영합니다. 각 행은 해당 배치에 직접 연결한 구매·운전비만 표시하여 상류 배치와 중복되지 않습니다. 최종 배분 비율은 후속 분취를 모두 반영합니다. 간접비·판매 마진은 제외합니다.')}</p>
      <table className="mt-2 w-full text-left text-xs"><thead><tr>{[l('Intermediate → destination', '중간 생성물 → 투입 대상'), l('Recovered (kg)', '회수 (kg)'), l('Transferred (kg)', '분취 (kg)'), l('Final allocation (%)', '최종 배분 (%)'), l('Own batch charges (USD)', '해당 배치 지출 (USD)'), l('Allocated (USD)', '배분 비용 (USD)')].map((s) => <th className="p-2" key={s}>{s}</th>)}</tr></thead><tbody>{report.intermediate_batches.map((batch) => {
