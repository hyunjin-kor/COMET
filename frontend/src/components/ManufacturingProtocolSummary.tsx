...
export default function ManufacturingProtocolSummary({ report, compact = false }: { report: ManufacturingReport; compact?: boolean }) {
  ...
  return <section className="mt-4 rounded-xl border border-teal-200 bg-white p-4">
    ...
    {!!report.intermediate_batches?.length && <div className="mt-4 overflow-x-auto"><p className="text-sm font-medium">{l('Intermediate cost allocation', '중간 배치 비용 배분')}</p>
      <p className="mt-2 text-xs leading-6 text-slate-600">{l('Proportional allocation retains unused cost with recoverable inventory; whole-batch charging assigns all expenditure to the receiving batch. Each row lists only its own operations and purchases, avoiding overlap with upstream rows. Final allocated fractions include all subsequent transfers. Overhead and margin are excluded.', '비례 배분은 미사용 비용을 회수 가능한 재고에 남기고, 전체 배치 방식은 전액을 투입 대상 배치에 반영합니다. 각 행은 해당 배치에 직접 연결한 구매·운전비만 표시하여 상류 배치와 중복되지 않습니다. 최종 배분 비율은 후속 분취를 모두 반영합니다. 간접비·판매 마진은 제외합니다.')}</p>
      <table className="mt-2 w-full text-left text-xs"><thead><tr>{[l('Intermediate → destination', '중간 생성물 → 투입 대상'), l('Prepared/recovered', '제조·회수량'), l('Transferred', '분취량'), l('Final allocation (%)', '최종 배분 (%)'), l('Own batch charges (USD)', '해당 배치 지출 (USD)'), l('Allocated (USD)', '배분 비용 (USD)')].map((s) => <th className="p-2" key={s}>{s}</th>)}</tr></thead><tbody>{report.intermediate_batches.map((batch) => {
        ...
        const destination = report.intermediate_batches?.find((item) => item.id === batch.destination_batch_id)?.name ?? l('Final batch', '최종 배치');
        const volume = batch.allocation_basis === 'volume_used';
        const whole = batch.allocation_basis === 'whole_batch';
        return <tr key={batch.id} className="border-t border-slate-100">{[`${batch.name} → ${destination} (${whole ? l('whole batch', '전체 배치') : volume ? l('volume used', '사용 부피') : l('mass used', '사용 질량')})`, whole ? '—' : v(volume ? batch.produced_volume_ml : batch.produced_mass_kg, volume ? 'mL' : 'kg'), whole ? '—' : v(volume ? batch.used_volume_ml : batch.used_mass_kg, volume ? 'mL' : 'kg'), v(batch.allocation_fraction == null ? null : batch.allocation_fraction * 100), v(incurred), v(allocated)].map((cell, i) => <td className="p-2" key={i}>{cell}</td>)}</tr>;
      })}</tbody></table>
