  ...
  return <div className="mt-4">
    ...
    {purchases.map((purchase, i) => {
      ...
      return <div key={i} className="mt-3 grid gap-3 rounded-lg bg-slate-50 p-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-xs text-slate-600">{l('Quantity and price unit', '구매량·단가의 공통 단위')}<select className="input-base mt-1 w-full" disabled={purchase.quantity_basis === 'solvent_volume'} value={purchase.unit} onChange={(e) => change({ unit: e.target.value as BatchPurchase['unit'] })}>{['kg', 'g', 'L', 'mL', 'mol', 'mmol', 'item'].map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label>
