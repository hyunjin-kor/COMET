...
export function SavedEstimateComparison({ savedEstimates, priceBasis }: Props) {
  ...
  return (
    <section className="mt-4 surface-ghost p-4" aria-label={t('Compare saved estimates')}>
      ...
      <div className="mt-3 grid max-h-56 gap-2 overflow-y-auto sm:grid-cols-2">
        {savedEstimates.map((saved) => (
          <label key={saved.id} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm">
            ...
            <span className="min-w-0 break-words"><ScientificText text={saved.name} /></span>
          </label>
        ))}
      </div>
        ...
        <label className="text-xs font-semibold text-slate-600">
          ...
          <select value={hasReference ? reference!.id : ''} disabled={busy}
            ...
            {savedEstimates.filter((item) => available.includes(item.id)).map((item) => (
              <option key={item.id} value={item.id}>{formatScientificText(item.name)}</option>
            ))}
          </select>
        </label>
        <label className="text-xs font-semibold text-slate-600">
      ...
      {error ? <p role="alert" className="mt-3 text-sm text-red-700"><ScientificText text={error} /></p> : null}
      {result && canShowResult ? (
        <div className="mt-4 space-y-4">
          <p className="text-xs leading-6 text-slate-600"><ScientificText text={result.unit === 'USD/cm2' ? t('Comparison metric: electrode assembly cost per area.') : t('Comparison metric: selling price including margin, less recovery value.')} /></p>
            ...
            <table className="w-full min-w-[620px] bg-white text-left text-xs">
              ...
              <tbody>{result.estimates.map((row) => (
                <tr key={row.estimate_id} className="border-t border-slate-200">
                  <th className="p-3 font-medium"><ScientificText text={row.name} /><span className="mt-1 block text-slate-500"><ScientificText text={String(row.common_conditions.input_summary.composition ?? '')} /></span><span className="mt-1 block text-slate-500"><ScientificText text={row.common_conditions.route_summary?.name ?? t('Custom steps')} /></span></th>
                  <td className="whitespace-nowrap p-3"><ScientificText text={displayPrice(row.values.saved)} /></td>
                  <td className="whitespace-nowrap p-3"><ScientificText text={displayPrice(row.values.repriced_original_conditions)} /></td>
                  <td className="whitespace-nowrap p-3 font-semibold"><ScientificText text={displayPrice(row.values.common_conditions)} /></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-6 text-amber-900">
            ...
            {result.estimates.map((row) => <details key={row.estimate_id} className="mt-2">
              ...
              {row.common_conditions.costing_scope ? <>
                <p>{t('Costing status')}: <ScientificText text={row.common_conditions.costing_scope.status} /></p>
                <p><ScientificText text={row.common_conditions.costing_scope.boundary} /></p>
                <p>{t('Uncosted operations')}: <ScientificText text={row.common_conditions.costing_scope.uncosted_operations.join(', ') || t('None declared')} /></p>
                <p>{t('Scale substitutions')}: <ScientificText text={row.common_conditions.costing_scope.substitutions.map((item) => `${item.from} → ${item.to}`).join(', ') || t('None declared')} /></p>
              </> : null}
              {(row.common_conditions.warnings ?? []).map((warning) => <p key={warning}><ScientificText text={warning} /></p>)}
            </details>)}
          </div>
          <details className="rounded-xl border border-slate-200 bg-white p-3 text-xs">
            ...
            {result.price_snapshot.map((price) => <div key={price.key} className="mt-2 border-t border-slate-100 pt-2 break-words">
              <p className="font-medium"><ScientificText text={price.key} /></p>
              <p><ScientificText text={Object.entries(price.values).map(([key, value]) => `${key}: ${value}`).join(' · ')} /></p>
