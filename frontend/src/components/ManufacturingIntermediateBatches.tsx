...
export default function ManufacturingIntermediateBatches({ value, onChange }: {
  ...
  return <details className="rounded-xl border border-slate-200 p-4" open={batches.length > 0 || undefined}>
    ...
    <p className="mt-3 text-xs leading-6 text-slate-600">{l(
      'Assign preparation operations and a receiving batch. Allocate by used/recovered mass, or used/prepared volume for a homogeneous stock solution at the same concentration. Successive fractions multiply. Leave unreported amounts blank; no density or yield is inferred.',
      '제조 단계와 투입 대상 배치를 연결하세요. 사용/회수 질량 비율 또는 농도가 같은 균일 용액의 사용/제조 부피 비율로 배분합니다. 분취가 이어지면 각 비율을 곱합니다. 미보고량은 비워 두며, 밀도나 수득량을 추정하지 않습니다.')}</p>
    <p className="mt-2 text-xs leading-6 text-slate-600">{l(
      'Proportional allocation leaves the unused share with recoverable inventory; whole-batch charging assigns all costs to the receiving batch. Each intermediate has one destination; splits across multiple branches and co-products are not modeled. Do not enter internal transfers as purchases.',
      '비례 배분은 남은 비용을 회수 가능한 재고에 남기고, 전체 배치 방식은 전액을 투입 대상 배치에 부담시킵니다. 각 중간 배치는 한 곳에만 연결하며 여러 경로로 나누는 분기와 공동생산물은 계산하지 않습니다. 내부 이동량을 구매로 다시 넣지 마세요.')}</p>
    {batches.map((batch, index) => {
      ...
      const quantityFields = batch.allocation_basis === 'volume_used' ? ['produced_volume_ml', 'used_volume_ml'] as const : ['produced_mass_kg', 'used_mass_kg'] as const;
      const quantityLabels = { produced_mass_kg: l('Mass recovered (kg)', '회수 질량 (kg)'), used_mass_kg: l('Mass transferred (kg)', '분취 질량 (kg)'), produced_volume_ml: l('Stock solution prepared (mL)', '제조한 원액 부피 (mL)'), used_volume_ml: l('Stock solution transferred (mL)', '분취한 원액 부피 (mL)') };
      return <div key={batch.id} className="mt-3 rounded-lg bg-slate-50 p-3">
        ...
        <label className="mt-3 block text-xs">{l('Cost allocation', '비용 배분 기준')}<select className="input-base mt-1 w-full" value={batch.allocation_basis ?? 'mass_used'} onChange={(e) => update(batch.id, { allocation_basis: e.target.value as IntermediateBatch['allocation_basis'] })}><option value="mass_used">{l('Used mass fraction; remainder held in inventory', '사용 질량 비율 · 잔여분은 재고')}</option><option value="volume_used">{l('Used volume fraction of homogeneous solution', '균일 용액의 사용 부피 비율')}</option><option value="whole_batch">{l('Charge whole batch; no inventory credit', '전체 배치 비용 · 재고 공제 없음')}</option></select></label>
        ...
        {batch.allocation_basis !== 'whole_batch' && <div className="mt-3 grid gap-3 sm:grid-cols-2">{quantityFields.map((key) => <label key={key} className="text-xs">{quantityLabels[key]}<input type="number" min="0" step="any" className="input-base mt-1 w-full" value={batch[key] ?? ''} onChange={(e) => update(batch.id, { [key]: e.target.value === '' ? null : Number(e.target.value) })} /></label>)}</div>}
        ...
        <ManufacturingInputSources record={batch} fields={['allocation_basis', 'destination_batch_id', ...quantityFields, 'notes']} onChange={(input_evidence) => update(batch.id, { input_evidence })} />
