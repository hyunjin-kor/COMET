...
export default function ManufacturingIntermediateBatches({ value, onChange }: {
  ...
  return <details className="rounded-xl border border-slate-200 p-4" open={batches.length > 0 || undefined}>
    ...
    <p className="mt-3 text-xs leading-6 text-slate-600">{l(
      'When only part of a support or intermediate batch is used, assign its preparation operations to that batch. Costs are allocated by mass used / mass recovered, including purchases and operating costs. Use the same material and mass basis for both values. Leave unreported recovery blank.',
      '합성한 담체나 중간 생성물의 일부만 사용했다면, 그 제조 단계를 중간 배치에 연결하세요. 구매비와 운전비를 사용량/회수량 비율로 배분합니다. 두 질량은 같은 물질·질량 기준이어야 합니다. 회수량이 보고되지 않았으면 비워 두세요.')}</p>
    <p className="mt-2 text-xs leading-6 text-slate-600">{l(
      'Each intermediate feeds the final batch directly. Mass allocation leaves the unused share with recoverable inventory; choose whole-batch charging when this final batch bears all expenditure. Nested transfers and co-products are not modeled. Do not enter the internal transfer as another purchase.',
      '각 중간 배치는 최종 배치에 직접 투입됩니다. 질량 배분은 남은 비용을 회수 가능한 재고에 남깁니다. 지출 전액을 최종 배치에 부담시키려면 전체 배치 비용을 선택하세요. 중간 배치 간 연속 분취와 공동생산물은 계산하지 않습니다. 내부 이동량을 구매 항목에 다시 넣지 마세요.')}</p>
    {batches.map((batch, index) => {
      const assigned = value.operations.some((op) => op.intermediate_batch_id === batch.id);
      return <div key={batch.id} className="mt-3 rounded-lg bg-slate-50 p-3">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">{(['produced_mass_kg', 'used_mass_kg'] as const).map((key) => <label key={key} className="text-xs">{key === 'produced_mass_kg' ? l('Mass recovered from this batch (kg)', '이 배치에서 회수한 질량 (kg)') : l('Mass used in final batch (kg)', '최종 배치에 사용한 질량 (kg)')}<input type="number" min="0" step="any" className="input-base mt-1 w-full" value={batch[key] ?? ''} onChange={(e) => update(batch.id, { [key]: e.target.value === '' ? null : Number(e.target.value) })} /></label>)}</div>
        ...
        <ManufacturingInputSources record={batch} fields={['allocation_basis', 'produced_mass_kg', 'used_mass_kg', 'notes']} onChange={(input_evidence) => update(batch.id, { input_evidence })} />
        ...
        <p className="mt-2 text-xs text-slate-500">{assigned ? l('Move its operations to another batch before removing this entry.', '삭제하려면 연결된 제조 단계의 배치를 먼저 바꾸세요.') : l('Select this batch in the preparation operations below.', '아래 제조 단계에서 이 배치를 선택하세요.')}</p>
      </div>;
    })}
