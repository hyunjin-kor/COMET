...
COMET (Catalyst Overall Manufacturing Estimation Tool) integrates catalyst manufacturing cost estimation, preparation records, price management, and candidate ranking. A library of {rb('summary.candidates')} formulations in {rb('summary.families')} reaction families supports comparisons under explicit assumptions. Source-linked preparation records retain experimental conditions and distinguish published values from user modifications. Batch calculations trace purchases, electricity, gas, labor, equipment occupancy, and intermediate transfers to cost per kilogram of dry product. Saved inputs support sensitivity analysis and reproducible price updates. Published examples verify the Step Method implementation; an illustrative batch demonstrates the effects of treatment duration and recovered mass. Independent industrial validation remains necessary.
...
Preparation evidence, candidate assessments, operating references, calculation methods, manufacturing inputs and sensitivity results, historical prices, and reproduction instructions (PDF and machine-readable files).

## Data and Software Availability

COMET version {r('m', 'project_version')} uses the PolyForm Noncommercial License 1.0.0; commercial use requires a separate license. Repository: https://github.com/hyunjin-kor/COMET. Concept DOI: 10.5281/zenodo.21451931. [Authors must confirm access to this version and analysis files before submission.] Analysis inputs, outputs, checksums, and scripts accompany the study. Third-party data retain their source terms.
