...
COMET is a local application with no account. The backend is a FastAPI service on Python 3.11 or newer, using SQLModel over a single SQLite file for prices, saved estimates, purchase evidence and actual-cost observations. The user interface is a React 19 and TypeScript single-page application in Korean and English, with scientific notation for formulas and units. On Windows an Electron shell bundles the backend with PyInstaller and installs as `COMET.Setup.<version>.exe`, with a portable archive as an alternative. The same application can be served to a browser from source, with the interface built once and served by the backend on a local port (`npm run web` on Windows). The backend tests and the interface build run on Linux in continuous integration, but the authors have exercised the complete browser mode only on Windows. Prices refresh on start-up and once a day; without API keys the software uses index-adjusted and user-entered prices and works offline.
...
COMET estimates costs with an adopted method; it does not measure them. No public observation matched a candidate's composition, grade, order size, date and cost boundary closely enough to compute an empirical error, so the mean absolute percentage error of the model against industrial purchases is unestimated, which is not the same as zero. The library candidates are literature architectures and engineering proxies with author-assigned readiness and performance scores; those scores are inputs to be questioned, and the score sensitivity tests exist for that purpose. Environmental figures are partial inventories: mean materials coverage across the library is {r('s', 'lca.coverage_mean_pct', '.2f')}% and {r('s', 'lca.candidates_coverage_below_50_pct')} candidates fall below half coverage, while solvent supply, wastewater and equipment manufacture are outside the process term. Manufacturing rates come from a 2017 equipment table; operations absent from that table stay uncosted. Cost-responsive synthesis and lifetime economics have been studied elsewhere;<sup>13,14</sup> the software does not model activity, deactivation or use-phase impacts, and a cheaper catalyst is not a better one.

## Data and Software Availability

Source code, the Windows installer and the portable archive are at https://github.com/hyunjin-kor/COMET under the PolyForm Noncommercial License 1.0.0, which permits noncommercial use; the license is not OSI-approved, and commercial licences are available on request from the corresponding author. The repository is archived under the concept DOI 10.5281/zenodo.21451931, and this note describes version {r('m', 'project_version')}; beyond Windows, the application runs in a browser from source with Python 3.11 or newer and Node.js 22 or newer, with the platform caveat stated above. The frozen price snapshots, the analysis outputs behind every number and figure, their SHA-256 manifests and the commands that regenerate them are in the repository's paper directory. Metal prices and trade statistics come from the public sources cited in the text and can be re-extracted with the collection scripts in the same repository.
