...
The same study reports market prices for the three catalysts.<sup>1</sup> Relative to those prices, COMET estimates differ by {mk('[0].market.comet_vs_market_pct', '.1f')}% for Pt/C and {mk('[1].market.comet_vs_market_pct', '.1f')}% for Ni/Al₂O₃. The estimate for the FCC catalyst is {mk('[2].with_published_rate.estimated_price_per_lb', '.4f')} USD/lb, compared with a reported market price of {mk('[2].market.market_price_per_lb', '.2f')} USD/lb (Figure 2c). Both COMET and the published estimates are below the reported market prices. The original study obtained these prices through communication with industry experts and reported agreement within ±20%; COMET's estimates also fall within that range. These comparisons do not establish the source of the residual differences or accuracy for other formulations. A separate comparison uses U.S. import unit values for HS 3815 catalyst categories.<sup>10</sup> Figure 2d shows the ratio of estimated price to import unit value for the candidate ranked first at baseline in each of 23 reaction families. The ratios vary substantially with family and month. Because the trade data combine products with different grades, loadings, formulations, and order sizes, they do not provide matched observations for validating individual formulations.
