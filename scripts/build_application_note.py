...
To test whether cost orderings persist, every family was recalculated for {cs(monthly + '.observations')} monthly metal-price states with all other inputs and non-cost scores fixed. The lowest-cost candidate changes in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families. In ammonia cracking (Figure 4d), {co} is least expensive in {cs(monthly + '.winner_counts.cost_winner.co-mgo-la2o3')} states and {ni_al} in {cs(monthly + '.winner_counts.cost_winner.ni-alumina-baseline')}. From September to October 2025, cobalt rose from {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_before', '.2f')} to {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_after', '.2f')} USD/kg with nickel nearly unchanged, and {ni_al} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.ni-alumina-baseline', '.2f')} USD/kg) became cheaper than {co} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.co-mgo-la2o3', '.2f')} USD/kg). Monthly states cluster near the equal-cost boundary in the nickel–cobalt price plane (Figure 4e), so modest cobalt moves change the cost leader. Not every ordering is price-sensitive: {r(WHATIF, 'catalysts.ru.loading_wt_pct', '.0f')} wt% Ru/Al₂O₃ ({wi('catalysts.ru.baseline.selling_price_per_lb', ',.2f')} USD/kg) would match the Ni/Al₂O₃ catalyst only if ruthenium fell to {equal_pct}% of its {basis} price, below every month of the {r(WHATIF, 'equal_cost.ru_history_months')}-month record.

![Figure 4. What-if analyses and observed-price crossovers. (a) Selling price versus metal loading for Ni/Al₂O₃ and Ru/Al₂O₃ prepared by incipient wetness impregnation; labels give the change per wt%. (b) Selling price versus order size; dotted lines separate the production scales. (c) Selling price of 20 wt% Ni/Al₂O₃ for five preparation procedures, itemized as materials, processing, G&A, SARD, and margin. (d) Costs of the ammonia-cracking candidates that attain the lowest cost under {cs(monthly + '.observations')} monthly price states. (e) Conditional equal-cost boundary between {co} and {ni_al} in the nickel–cobalt price plane; points are monthly states, the arrow marks the move from September to October 2025 with the cobalt price in USD/kg, and shading identifies the cheaper candidate.](figures-note-2026-09-09/fig4_decision_diagnostics.png)
