...
The library holds {rb('summary.candidates')} formulations in {rb('summary.families')} reaction families ({thermal} of powder catalysts, {electro} of electrocatalysts), each with a composition, procedure, order size, and route and performance scores. Compositions follow a literature catalyst architecture for {r('s', 'screening_basis_counts.literature_architecture_proxy')} candidates, engineering baselines for {r('s', 'screening_basis_counts.engineering_proxy')}, and vendor or market data for {other_bases}; all are screening models, not verified recipes, and their estimates add assumed route allowances (Figure 2c; Table S7). Preparation records link candidates to what their sources report: an audit verified {r(PREPARATION, 'crossref_dois')} digital object identifiers (DOIs) with Crossref and transcribed {r(PREPARATION, 'profiles')} specimen-specific records from {r(PREPARATION, 'primary_sources')} primary sources for {r(PREPARATION, 'candidates_with_profile')} candidates, flagging composition or source discrepancies in {r(PREPARATION, 'source_mismatches')}. Each value keeps its DOI and locator, and unreported conditions stay blank. A record documents a source; it does not validate the library formulation (Section S6).

### Prices and ranking

Prices are stored with source, date, and retrieval time. The reference basis uses monthly averages from the International Monetary Fund (IMF) Primary Commodity Price System<sup>10</sup> and Johnson Matthey<sup>8</sup> for metals and U.S. import unit values for supports<sup>11</sup> (Section S5). Results use the {basis} reference basis, and the same series give {rb('summary.months')} monthly price states from {first}. The live basis uses current quotations, for example from Johnson Matthey<sup>8</sup> and Westmetall.<sup>9</sup> A price-reliability score averages source grades by share of materials cost; it rates price provenance, not estimate accuracy.

Within a family, candidates are ranked by a weighted sum of four 0–100 scores: cost, price reliability, route, and performance. The cost score is min–max normalized from 100 (least expensive) to 0 (most expensive); route and performance scores are assigned from the literature. The balanced weight profile used below gives cost the largest weight ({weight_low}–{weight_high}).

## Results and Discussion

### Verification

The source of the Step Method tabulates three demonstration cases (Pt/C, Ni/Al₂O₃, and a fluid catalytic cracking (FCC) catalyst) with every intermediate line and a market price.<sup>1</sup> Their inputs were entered as published. For Pt/C and Ni/Al₂O₃, COMET reproduces every line before the margin within {premargin_dev}%, the rounding of the published table (Table S1), and the Pt/C price matches ({kg('s', 'table62[0].comet_usd_per_lb', '.2f')} USD/kg, platinum value excluded). The Ni/Al₂O₃ price is {kg('s', 'table62[1].comet_usd_per_lb', '.2f')} against {kg('s', 'table62[1].published_usd_per_lb', '.2f')} USD/kg ({minus(r('s', 'table62[1].residual_pct', '+.2f'))}%), and the whole difference lies in the margin: the table applies {r(MARKET, '[1].margin.table_footnote_pct_of_premargin', '.0f')}% of pre-margin cost, whereas COMET applies the order-size correlation of the same source, which gives {r(MARKET, '[1].margin.comet_pct_of_premargin', '.0f')}%. For FCC, costed at the effective production rate that the source states ({kg('s', 'table62[2].effective_rate_ton_per_day', ',.1f', KG_PER_SHORT_TON)} kg/day), COMET gives {kg('s', 'table62[2].comet_usd_per_lb', '.2f')} against {kg('s', 'table62[2].published_usd_per_lb', '.2f')} USD/kg ({r('s', 'table62[2].residual_pct', '+.2f')}%). Against the market prices of the source, the COMET estimates deviate by {minus(mk('[0].market.comet_vs_market_pct', '.1f'))}%, {minus(mk('[1].market.comet_vs_market_pct', '.1f'))}%, and {minus(fcc_market_pct)}% (Figure 2b), within the ±20% reported for the method. These results verify the implementation; they do not validate new estimates, because the market comparison covers three catalysts at mid-2017 prices.

### Production-scale cost structure

Figure 2c divides the selling price of the lowest-cost candidate in each of the {thermal} powder-catalyst families. Materials exceed half of the price in only {materials_majority} families, processing exceeds materials in {processing_over}, and G&A, SARD, margin, and route allowances take {other_low}–{other_high}%. Most estimates thus depend on processing assumptions as much as on materials prices. The Step Method supplies these assumptions as standard hourly step costs; for a laboratory preparation, they must come from the preparation itself.

### Laboratory batch costs

Published preparations rarely report the operating inputs that batch costing needs. In the {r(PREPARATION, 'profiles')} preparation records, all {n_segments} heating segments state a target temperature, {n_hold} a hold time, and {n_ramp} a ramp rate, but none states the electrical input, and only {n_mass} record reports the recovered batch mass. Figure 3 shows what such omissions are worth for an illustrative {sn('[0].value', '.3f')} kg batch costed from declared, assumed inputs (Tables S2–S5). Operations account for {r(MANUFACTURING, 'baseline.summary.processing_pct', '.1f')}% of the baseline price of {r(MANUFACTURING, 'baseline.summary.estimated_price_per_kg', ',.2f')} USD/kg (Figure 3b). Duration outweighs power: calcination holds of {sn('[4].low', '.0f')}–{sn('[4].high', '.0f')} h span {sn('[4].low_usd_kg', ',.2f')}–{sn('[4].high_usd_kg', ',.2f')} USD/kg, whereas hold powers of {sn('[5].low', '.1f')}–{sn('[5].high', '.1f')} kW span only {sn('[5].low_usd_kg', ',.2f')}–{sn('[5].high_usd_kg', ',.2f')} USD/kg (Table S6). Recovered mass matters most because every cost is divided by it: halving the dry output doubles the price to {sn('[0].low_usd_kg', ',.2f')} USD/kg (Figure 3c). Of the two dominant inputs, hold time is usually reported, but recovered mass almost never is.
...
At production scale, the Step Method derives operating inputs from the procedure and order size, leaving composition, procedure, and order size to the user. Each was varied in turn for two alumina-supported catalysts prepared by incipient wetness impregnation (Figure 4a–c; Table S8). The base case, {r(EXAMPLE, 'request.components[0].wt_pct', '.0f')} wt% Ni/Al₂O₃ ordered at {kg(EXAMPLE, 'request.order_size_tons', ',.1f', KG_PER_SHORT_TON)} kg, sells for {kg(EXAMPLE, 'step_method.estimated_price_per_lb', '.2f')} USD/kg. Raising the nickel loading from {r(WHATIF, 'loading.ni[0].loading_wt_pct', '.0f')} to {r(WHATIF, 'loading.ni[5].loading_wt_pct', '.0f')} wt% raises the price from {wi('loading.ni[0].selling_price_per_lb')} to {wi('loading.ni[5].selling_price_per_lb')} USD/kg ({slopes['ni']} USD/kg per wt%), whereas each wt% of ruthenium adds {slopes['ru']} USD/kg. Order size acts in the opposite direction: from {wi('order_size.ni[0].order_size_tons', ',.1f', KG_PER_SHORT_TON)} to {wi('order_size.ni[10].order_size_tons', ',.1f', KG_PER_SHORT_TON)} kg, the Ni/Al₂O₃ price falls {folds['ni']}-fold, from {wi('order_size.ni[0].selling_price_per_lb')} to {wi('order_size.ni[10].selling_price_per_lb')} USD/kg, because a larger order moves production to a larger scale and lowers the margin; the price of {r(WHATIF, 'catalysts.ru.loading_wt_pct', '.0f')} wt% Ru/Al₂O₃ falls only {folds['ru']}-fold because materials dominate it. Five procedures for the same Ni/Al₂O₃ composition span {wi('preparation[0].selling_price_per_lb')}–{wi('preparation[2].selling_price_per_lb')} USD/kg. For the base-metal catalyst, order size thus outweighs loading, and loading outweighs procedure; for the precious-metal catalyst, loading outweighs both.

### Observed price crossovers

Metal prices change without any decision by the user, so every family was repriced at each of the {cs(monthly + '.observations')} monthly price states with all other inputs and the non-cost scores fixed. The lowest-cost candidate changes in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families (Figure 4e). In ammonia cracking, {co} is the least expensive candidate in {cs(monthly + '.winner_counts.cost_winner.co-mgo-la2o3')} months and {ni_al} in {cs(monthly + '.winner_counts.cost_winner.ni-alumina-baseline')} (Figure 4d). From September to October 2025, cobalt rose from {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_before', '.2f')} to {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_after', '.2f')} USD/kg while nickel hardly moved, and {ni_al} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.ni-alumina-baseline', '.2f')} USD/kg) became cheaper than {co} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.co-mgo-la2o3', '.2f')} USD/kg). Such reversals recur because the monthly states lie close to the equal-cost line in the nickel–cobalt price plane (Figure S5c). A large cost gap is insensitive: {r(WHATIF, 'catalysts.ru.loading_wt_pct', '.0f')} wt% Ru/Al₂O₃ ({wi('catalysts.ru.baseline.selling_price_per_lb', ',.2f')} USD/kg) would match the Ni/Al₂O₃ base case only at {equal_pct}% of the {basis} ruthenium price, below all {r(WHATIF, 'equal_cost.ru_history_months')} months. A cost order between candidates of similar cost should therefore carry its price date.

![Figure 4. What-if analyses and observed-price crossovers. (a) Selling price versus metal loading for Ni/Al₂O₃ and Ru/Al₂O₃ prepared by incipient wetness impregnation; labels give the change per wt%. (b) Selling price versus order size; dotted lines separate the production scales. (c) Selling price of 20 wt% Ni/Al₂O₃ for five preparation procedures, itemized as materials, processing, G&A, SARD, and margin. (d) Costs of the two ammonia-cracking candidates that attain the lowest cost under {cs(monthly + '.observations')} monthly price states. (e) Lowest-cost candidate of each month in the {cs('summary.monthly.cost_winner')} families where it changes; candidate names carry the color of their months. Ni/CeO₂ denotes Ni/CeO₂ single sites; the electrocatalyst families are compared on powder cost (Table S7).](figures-note-2026-09-09/fig4_decision_diagnostics.png)

### Recommendation sensitivity

A change in cost order need not change the recommendation. In ammonia cracking, the balanced-profile leader remains {co} in all {cs(monthly + '.winner_counts.app_winner.co-mgo-la2o3')} months because the cost score is normalized within the family: the precious-metal candidate {ru}, at {kg(METHODS, 'normalization.example.removed_cost', ',.2f')} USD/kg, fixes the expensive end of the scale, so the two cost leaders keep cost scores of at least {compressed} and the other scores decide their order. The same normalization ties the recommendation to the candidate set: removing {ru} contracts the scale, magnifies the {pair_gap} USD/kg difference between the two, and reverses their ranking, a known property of min–max normalization;<sup>6,7</sup> retaining the original range prevents the reversal. Over all families, removing one candidate that does not rank first changes the leader in {rb('summary.candidate_removal_winner_changes')} of {rb('summary.candidate_removal_cases')} tests (Figure S6). The price basis matters as well: live quotations collected on {live_date} change the leader in {r('s', 'live_reference_comparison.changed_by_profile.balanced')} of {cs('summary.monthly.families')} families, partly because metals without a live quotation, such as cobalt, fall back to annual prices with a lower reliability grade (Table S9). A recommendation should therefore carry its candidate set, price basis, and date.
...
COMET implements the Step Method and records, with each estimate, its input sources, price date, and candidate set. It reproduces the three published examples line by line; the largest price difference, {largest_residual}%, arises from the margin rule. Order size sets the estimated cost of a base-metal catalyst, metal loading that of a precious-metal catalyst, and treatment duration and recovered mass that of a laboratory batch, although only {n_mass} of {r(PREPARATION, 'profiles')} preparation records reports the recovered mass. Observed metal prices changed the lowest-cost candidate in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families, and recommendations changed with the candidate set and the price basis. Cost comparisons should therefore state order size, assumed operating inputs, price basis and date, and candidate set. Because COMET does not evaluate performance, we plan to couple it with catalyst performance-prediction models, so that cost per unit of product can be compared, and to validate it against industrial prices under matched conditions.
