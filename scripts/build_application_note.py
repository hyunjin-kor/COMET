...
Table 6.2 also prints a market price for each catalyst, which separates two questions: whether the implementation reproduces the method, and how far the method sits from what a buyer pays. COMET lands {mk('[0].market.comet_vs_market_pct', '.1f')}% from the market price for platinum on carbon and {mk('[1].market.comet_vs_market_pct', '.1f')}% for nickel on alumina, and gives {mk('[2].with_published_rate.estimated_price_per_lb', '.4f')} against a market price of {mk('[2].market.market_price_per_lb', '.2f')} USD/lb for the cracking catalyst (Figure 2c). The published Step Method estimate sits a similar distance below the market price in each case, so the gap belongs to the adopted manufacturing-cost boundary, which stops before distribution, packaging and the seller's own margin, rather than to this implementation. The published method states agreement within ±20% of those market prices, which were taken from personal communications with catalyst industry experts rather than from a public price list;<sup>1</sup> all three COMET estimates fall inside that band. These checks establish that the method is reproduced and show its standing offset against three published market prices; they do not bound the error of a new formulation, for which no condition-matched industrial observation has been found.
...
Source code, the Windows installer and the portable archive are at https://github.com/hyunjin-kor/COMET under the PolyForm Noncommercial License 1.0.0, which permits noncommercial use; the license is not OSI-approved, and commercial licenses are available on request from [contact to be supplied by the authors]. The repository is archived under the concept DOI 10.5281/zenodo.21451931. The prepared version is {r('m', 'project_version')}. Windows users run the installer; on other systems the application is served to a browser from source with Python 3.11 or newer and Node.js 22 or newer, as described in the README, with the platform caveat stated above. The backend test suite runs with `python -m pytest backend/tests -q`, and the analyses in this note are regenerated with the commands recorded in the repository's paper index.
