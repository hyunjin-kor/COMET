...
def note(run):
    ...
    other_bases = sum(value for key, value in basis_counts.items() if key not in ("literature_architecture_proxy", "engineering_proxy"))
    ...
    fcc_market_pct = 100 * (fcc["with_published_rate"]["estimated_price_per_lb"] / fcc["market"]["market_price_per_lb"] - 1)
...
Figure 2(b) shows the lowest-cost screening candidate in each of 23 thermal families at May 2026 prices, including assumed route allowances. Bars total 100%, are ordered by materials share, and include absolute prices; comparisons across reactions do not establish equivalent performance. At these prices, a 20 wt% Ni/Al₂O₃ scenario prepared by incipient wetness impregnation, ordered at {kg(EXAMPLE, 'request.order_size_tons', ',.1f', KG_PER_SHORT_TON)} kg with alumina priced by import unit value, sells for {kg(EXAMPLE, 'step_method.estimated_price_per_lb', '.2f')} USD/kg; nickel, alumina, and processing contribute {kg(EXAMPLE, 'materials.components[0].cost_per_lb_cat', '.2f')}, {kg(EXAMPLE, 'materials.components[1].cost_per_lb_cat', '.2f')}, and {kg(EXAMPLE, 'step_method.processing_cost_per_lb', '.2f')} USD/kg.
...
Current quotations include Johnson Matthey platinum and palladium prices<sup>7</sup> and Westmetall copper and aluminum settlements.<sup>8</sup> Historical inputs use International Monetary Fund (IMF) Primary Commodity Price System data<sup>9</sup> and monthly Johnson Matthey averages. Support-material prices use U.S. import unit values<sup>10</sup> for {r('meta', 'support_series')} commodity codes and {r('meta', 'support_observations')} verified observations. Retrieval times and source hierarchies are retained. Supporting Information plots the unsmoothed metal histories; these are observations, not forecasts. Price-source grades are weighted by materials-cost contributions and assess pricing provenance, not preparation or performance verification.

The screening library includes {r('s', 'screening_basis_counts.literature_architecture_proxy')} literature-architecture proxies, {r('s', 'screening_basis_counts.engineering_proxy')} engineering proxies, and {other_bases} candidates with individually documented vendor or market bases. Rankings combine cost, price-data reliability, route, and performance; in rankings and price replays, cost denotes the modeled selling price P per kilogram. Within each family, cost scores span 100 for the least expensive candidate to 0 for the most expensive; equal costs receive 100. Route and performance scores are author-assigned screening judgments. Baseline rankings use May 2026 prices, balanced weights, and the full candidate set; lower cost resolves ties. Electrochemical families whose candidates lack complete electrode data are compared on catalyst-powder costs.

Observed prices can reverse cost ordering. Figure 4(a) recalculates the ammonia-cracking (ammonia decomposition) candidates under {cs(monthly + '.observations')} monthly metal-price states with formulations, order sizes, route assumptions, support prices, price-source grades, and route and performance scores fixed: {co} is least expensive in {cs(monthly + '.winner_counts.cost_winner.co-mgo-la2o3')} states and {ni_al} in {cs(monthly + '.winner_counts.cost_winner.ni-alumina-baseline')}, while the balanced-weight recommendation does not change. Across all {cs('summary.monthly.families')} families, the lowest-cost candidate changes in {cs('summary.monthly.cost_winner')} and the balanced leader in {cs('summary.monthly.app_winner')} within this window. Between September and October 2025, cobalt rose from {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_before', '.2f')} to {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_after', '.2f')} USD/kg with nickel nearly unchanged, and the {ni_al} cost of {kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.ni-alumina-baseline', '.2f')} USD/kg fell below the {co} cost of {kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.co-mgo-la2o3', '.2f')} USD/kg. Figure 4(b) shows the conditional equal-cost boundary in the nickel–cobalt price plane; monthly states cluster near it, so modest cobalt movements change the cost leader. These are model comparisons under fixed formulations, not performance comparisons, and a cost crossover is distinct from a change in the composite recommendation.

Price-source grades also affect rankings. Replacing the May 2026 reference with the stored live quotations collected on {live_date} changes the balanced leader in {r('s', 'live_reference_comparison.changed_by_profile.balanced')} of {cs('summary.monthly.families')} families (Table S7). In ammonia cracking, cobalt has no stored live quotation, so the application uses an annual reference price; the price-reliability score of {co} falls from {score(reference_families, run.names['a'], 'co-mgo-la2o3', 'evidence')} to {score(live_families, LIVE_FAMILIES, 'co-mgo-la2o3', 'evidence')}, cost scores remain near 100, and the {ni_mgo} leads. Results should therefore be reported with their price basis and date.
...
Three published examples verify Step Method calculations without adjusting materials totals, sequences, or order sizes. COMET estimates {kg('s', 'table62[0].comet_usd_per_lb', '.4f')} USD/kg for Pt/C versus {kg('s', 'table62[0].published_usd_per_lb', '.3f')} USD/kg reported; both exclude platinum metal value. For Ni/Al₂O₃, {kg('s', 'table62[1].comet_usd_per_lb', '.4f')} versus {kg('s', 'table62[1].published_usd_per_lb', '.3f')} USD/kg differs by {minus(r('s', 'table62[1].residual_pct', '+.2f'))}% because COMET uses the published margin correlation instead of the table's fixed margin. For ultrastable zeolite Y in fluid catalytic cracking, {kg('s', 'table62[2].comet_usd_per_lb', '.4f')} versus {kg('s', 'table62[2].published_usd_per_lb', '.3f')} USD/kg differs by {r('s', 'table62[2].residual_pct', '+.2f')}% at the reported effective rate of {kg('s', 'table62[2].effective_rate_ton_per_day', ',.1f', KG_PER_SHORT_TON)} kg/day. Reported values are conversions of the published per-pound prices, rounded after conversion. Intermediate processing and overhead calculations agree at the reported precision.

Figure 2(c) compares estimates with the same source's market prices using 100 × (estimate − market price)/market price.<sup>1</sup> COMET deviations are {minus(mk('[0].market.comet_vs_market_pct', '.1f'))}% for Pt/C and {minus(mk('[1].market.comet_vs_market_pct', '.1f'))}% for Ni/Al₂O₃. For FCC, {kg(MARKET, '[2].with_published_rate.estimated_price_per_lb', '.4f')} USD/kg versus {kg(MARKET, '[2].market.market_price_per_lb', '.2f')} USD/kg reported gives {minus(format(fcc_market_pct, '.1f'))}%. All are below the reported prices and within the original study's ±20% range; this does not validate other formulations. Automated checks additionally cover manufacturing arithmetic, incomplete records, transfers, API persistence, and sensitivity calculations.

Ammonia cracking illustrates candidate-set sensitivity. Baseline costs are {kg(METHODS, 'normalization.example.rows[0].cost', '.2f')} USD/kg for {co}, {kg(METHODS, 'normalization.example.rows[1].cost', '.2f')} for the {ni_mgo}, {kg(METHODS, 'normalization.example.rows[2].cost', '.2f')} for {ni_al}, and {kg(METHODS, 'normalization.example.removed_cost', ',.2f')} for {ru}. {co} initially leads, with score {ms('normalization.example.rows[0].total_before', '.1f')} versus {ms('normalization.example.rows[2].total_before', '.1f')} for {ni_al}. Removing {ru} leaves remaining costs unchanged but contracts their range from approximately {full_range:,.0f} to {retained_range:.1f} USD/kg. Renormalization magnifies the {pair_gap:.2f} USD/kg cost difference, changing the scores to {ms('normalization.example.rows[0].total_after', '.1f')} and {ms('normalization.example.rows[2].total_after', '.1f')}, respectively, and reversing the ranking. Retaining the original range prevents this reversal.<sup>11,12</sup> Figure 4(d) reports 100 × (C₁ − C₀)/C₀ for nine affected families, where C₀ and C₁ are costs of the leaders before and after removal under unchanged manufacturing assumptions.
