...
Figure 1 connects formulation, price selection, cost estimation, and candidate ranking. Users choose a thermal catalyst or electrode assembly, enter a formulation or load a library candidate, and edit the preparation route and scale. Composition checks precede calculation. The result links cost contributions to operation coverage, environmental inventory gaps, and source records. Saved cases retain inputs for recalculation; CSV exports retain results and price sources. Companion analysis files record system boundaries, functional units, checksums, software versions, and random seeds for the sensitivity studies.
...
Prices, estimates, and supporting records are stored locally in SQLite. The bilingual React 19/TypeScript interface connects to a FastAPI backend requiring Python 3.11 or later; source builds require Node.js 22 or later. Electron packages Windows builds with PyInstaller. Linux checks cover backend tests and interface builds; complete interface workflows were tested on Windows. Stored or user-supplied prices enable offline calculation without API keys.

In a saved calculation using spot quotations, a 20 wt% Ni/Al₂O₃ catalyst prepared by incipient-wetness impregnation at an order size of {kg(EXAMPLE, 'request.order_size_tons', ',.1f', KG_PER_SHORT_TON)} kg has an estimated selling price of {kg(EXAMPLE, 'step_method.estimated_price_per_lb', '.2f')} USD/kg. Nickel and processing account for {kg(EXAMPLE, 'materials.components[0].cost_per_lb_cat', '.2f')} and {kg(EXAMPLE, 'step_method.processing_cost_per_lb', '.2f')} USD/kg, respectively. Figure 2(b) shows the least expensive candidate in each of 23 thermal reaction families at May 2026 prices. Each bar totals 100%; the adjacent value gives the selling price in USD/kg. These screening totals add documented route allowances to the Step Method price. Rows are ordered by materials share, distinguishing material-dominated estimates from those more sensitive to processing assumptions. Equal bar lengths do not imply equal prices, and comparisons across reaction families do not assess equivalent catalytic performance.

![Figure 2. Cost estimation. (a) Calculation scheme. (b) Cost shares and selling prices in USD/kg. Other costs comprise overheads, margin, and route-specific allowances. (c) COMET and literature estimates relative to market prices reported by Baddour et al.<sup>1</sup> Deviation = 100 × (estimated price − market price)/market price; negative values indicate estimates below market prices. SCR, selective catalytic reduction.](figures-note-2026-09-09/fig2_cost_model.png)
...
Electrode estimates combine catalyst loading, ionomer, membrane, and substrate costs per active area. A selected manufacturing scenario can add equipment, labor, and facility costs; otherwise the area total covers materials only. Powder-route costs remain separate, and complete stack assembly is excluded.

Environmental screening uses published cradle-to-gate metal factors to estimate global warming potential and cumulative energy demand per kilogram of catalyst.<sup>6</sup> The calculator displays embodied material impacts; benchmark analyses additionally evaluate modeled route fuel and electricity. Missing factors and unmodeled operations remain explicit gaps. Element-based mappings for compounds are screening approximations, not compound-specific inventories. Reported coverage is the mass fraction with matched factors, including these mappings; even 100% coverage does not establish a complete life-cycle inventory. Missing impacts must not be interpreted as zero.
...
Price-source reliability grades are converted to scores and weighted by materials-cost contributions. This criterion, termed evidence in COMET, assesses pricing provenance, not catalytic performance; price changes can therefore alter both cost and reliability scores. User-entered cost observations are comparable only when currency, month, composition, grade, order size, production rate, operations, and process boundary match.
