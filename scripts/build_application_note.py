...
Estimating the manufacturing cost of a catalyst at the research stage requires operating conditions that synthesis reports seldom give and metal prices that change over time, and cost estimates rarely state which values they assume. COMET is desktop software built on the Step Method of Baddour et al., which prices a laboratory synthesis as a sequence of industrial processing steps. It adds source-linked preparation records, batch costing from stated operating conditions, metal price histories, and ranking sensitivity tests. It reproduces the method's three published examples line by line; the largest price difference, {largest_residual}%, arises from the margin rule. Production scale changes the price of Ni/Al₂O₃ {folds['ni']}-fold, metal loading controls that of Ru/Al₂O₃, and recovered mass and treatment time control laboratory batch cost. Over {rb('summary.months')} months of metal prices, the lowest-cost candidate changes in {cs('summary.monthly.cost_winner')} of {rb('summary.families')} reaction families. COMET estimates cost, not performance; coupling with performance-prediction models is planned.
...
Baddour et al. proposed the Step Method for this purpose: a laboratory synthesis is mapped onto industrial processing steps with standard hourly costs, and materials, overheads, and a scale-dependent margin are added. For three commercial catalysts, its estimates fell within ±20% of market prices.<sup>1</sup> CatCost implemented the method in spreadsheet and web tools,<sup>2</sup> and other studies costed individual synthesis routes<sup>3,4</sup> or propagated parameter uncertainty through techno-economic models.<sup>5</sup> These approaches take operating conditions and prices as given. When candidates come from the literature, the conditions that set processing cost are seldom reported, and an estimate that fills them without stating the values cannot be checked. Metal prices also move, so a cost order found on one date need not hold on another, and when cost is combined with other criteria, min–max normalized scores depend on which candidates are compared.<sup>6,7</sup>

This work makes three contributions. (1) COMET implements the Step Method, links every operating condition and price to its source, and costs laboratory batches from stated conditions; its calculation is verified against the published examples. (2) It identifies the variables that set the estimated cost at laboratory and production scale. (3) It measures how often the cost order and the recommended candidate change with observed metal prices, preparation method, and the candidate set.
...
The library holds {rb('summary.candidates')} formulations in {rb('summary.families')} reaction families ({thermal} of powder catalysts, {electro} of electrocatalysts), each with a composition, preparation method, production scale, and route and performance scores. Compositions follow a literature formulation for {r('s', 'screening_basis_counts.literature_architecture_proxy')} candidates, an engineering baseline for {r('s', 'screening_basis_counts.engineering_proxy')}, and vendor or market data for {other_bases}; all are screening models, not verified recipes, and their estimates add assumed route allowances (Figure 2c). Each candidate is priced at its library loading, from the cited source where reported and otherwise assumed, not at a common loading (Table S7). Preparation records link candidates to what their sources report: a review verified {r(PREPARATION, 'crossref_dois')} digital object identifiers (DOIs) with Crossref and transcribed {r(PREPARATION, 'profiles')} sample-specific records from {r(PREPARATION, 'primary_sources')} primary sources for {r(PREPARATION, 'candidates_with_profile')} candidates, noting composition or source discrepancies in {r(PREPARATION, 'source_mismatches')}. Each value carries its DOI and source section, unreported conditions stay blank, and a record documents a source without validating the library formulation (Section S6).

### Prices and ranking

The reference basis stores monthly averages from the International Monetary Fund (IMF) Primary Commodity Price System<sup>10</sup> and Johnson Matthey<sup>8</sup> for metals and U.S. import unit values for supports,<sup>11</sup> each with source, date, and retrieval time (Section S5). Results use the {basis} reference basis; the same series give {rb('summary.months')} monthly price states from {first}. A second basis uses current quotations such as Johnson Matthey's<sup>8</sup> and Westmetall's.<sup>9</sup> A price-reliability score averages source grades by share of materials cost; it rates source quality, not estimate accuracy.

Within a family, candidates are ranked by a weighted sum of four 0–100 scores: cost, price reliability, route, and performance. The cost score is min–max normalized from 100 (least expensive) to 0 (most expensive); route and performance scores are assigned from the literature. The balanced weighting used below gives cost the largest weight ({weight_low}–{weight_high}).

## Results and Discussion

### Verification

The source of the Step Method tabulates three demonstration cases (Pt/C, Ni/Al₂O₃, and a fluid catalytic cracking (FCC) catalyst) with every intermediate line and a market price,<sup>1</sup> and their data were entered as published. For Pt/C and Ni/Al₂O₃, COMET reproduces every line before the margin within {premargin_dev}%, the rounding of the published table (Table S1), and the Pt/C price matches ({kg('s', 'table62[0].comet_usd_per_lb', '.2f')} USD/kg, platinum value excluded). The Ni/Al₂O₃ price is {kg('s', 'table62[1].comet_usd_per_lb', '.2f')} against {kg('s', 'table62[1].published_usd_per_lb', '.2f')} USD/kg ({minus(r('s', 'table62[1].residual_pct', '+.2f'))}%), and the whole difference lies in the margin: the table applies {r(MARKET, '[1].margin.table_footnote_pct_of_premargin', '.0f')}% of pre-margin cost, whereas the scale correlation of the same source, which COMET applies, gives {r(MARKET, '[1].margin.comet_pct_of_premargin', '.0f')}%. For FCC, costed at the effective production rate the source states ({kg('s', 'table62[2].effective_rate_ton_per_day', ',.1f', KG_PER_SHORT_TON)} kg/day), COMET gives {kg('s', 'table62[2].comet_usd_per_lb', '.2f')} against {kg('s', 'table62[2].published_usd_per_lb', '.2f')} USD/kg ({r('s', 'table62[2].residual_pct', '+.2f')}%). Against the market prices of the source, the estimates deviate by {minus(mk('[0].market.comet_vs_market_pct', '.1f'))}%, {minus(mk('[1].market.comet_vs_market_pct', '.1f'))}%, and {minus(fcc_market_pct)}% (Figure 2b), within the ±20% reported for the method. These results verify the calculation, not new estimates: the market comparison covers three catalysts at mid-2017 prices.
...
Metal prices change without any user decision, so every family was repriced at all {cs(monthly + '.observations')} monthly price states, other conditions fixed. The lowest-cost candidate changes in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families (Figure 4e). In ammonia cracking, {co_loading} wt% {co} is the least expensive candidate in {cs(monthly + '.winner_counts.cost_winner.co-mgo-la2o3')} months and {ni_loading} wt% {ni_al} in {cs(monthly + '.winner_counts.cost_winner.ni-alumina-baseline')} (Figure 4d). From September to October 2025, cobalt rose from {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_before', '.2f')} to {kg(CROSSOVER_MECHANISMS, 'cases[0].metal_effects.Co.price_after', '.2f')} USD/kg while nickel hardly moved, and {ni_al} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.ni-alumina-baseline', '.2f')} USD/kg) became cheaper than {co} ({kg(CROSSOVER_MECHANISMS, 'cases[0].costs_after.co-mgo-la2o3', '.2f')} USD/kg). Such reversals recur because the monthly states lie near the equal-cost line in the nickel–cobalt price plane (Figure S5c). A large cost gap is insensitive: {r(WHATIF, 'catalysts.ru.loading_wt_pct', '.0f')} wt% Ru/Al₂O₃ ({wi('catalysts.ru.baseline.selling_price_per_lb', ',.2f')} USD/kg) would match the Ni/Al₂O₃ base case only at {equal_pct}% of the {basis} ruthenium price, below all {r(WHATIF, 'equal_cost.ru_history_months')} months. A cost order between similar-cost candidates should therefore carry its price date.
...
A change in cost order need not change the recommendation. In ammonia cracking, the first-ranked candidate under balanced weighting remains {co} in all {cs(monthly + '.winner_counts.app_winner.co-mgo-la2o3')} months: the cost score is normalized within the family, and {ru}, at {kg(METHODS, 'normalization.example.removed_cost', ',.2f')} USD/kg, fixes the expensive end, so the two least expensive candidates keep cost scores of at least {compressed} and the other scores decide their order. Removing {ru} contracts the scale, magnifies the {pair_gap} USD/kg difference between them, and reverses their ranking, a known property of min–max normalization<sup>6,7</sup> that retaining the original range prevents. Over all families, removing one lower-ranked candidate changes the first-ranked candidate in {rb('summary.candidate_removal_winner_changes')} of {rb('summary.candidate_removal_cases')} tests (Figure S6). Current quotations collected on {live_date} change the first-ranked candidate in {r('s', 'live_reference_comparison.changed_by_profile.balanced')} of {cs('summary.monthly.families')} families, partly because cobalt, without a current quotation, falls back to an annual price of lower reliability grade (Table S10). A recommendation should therefore carry its candidate set, price basis, and date.
...
COMET implements the Step Method, documents with each estimate its data sources, price date, and candidate set, and reproduces the three published examples line by line; the largest price difference, {largest_residual}%, arises from the margin rule. Production scale sets the estimated cost of a base-metal catalyst, metal loading that of a precious-metal catalyst, and treatment time and recovered mass that of a laboratory batch, which only {n_mass} of {r(PREPARATION, 'profiles')} preparation records reports. Observed metal prices changed the lowest-cost candidate in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families, the preparation method could change it in {r(WHATIF, 'route_assignment.summary.lowest_cost_depends_on_template')} of {r(WHATIF, 'route_assignment.summary.families')}, and recommendations changed with the candidate set and the price basis. Cost comparisons should therefore state composition, preparation method, production scale, assumed operating conditions, price basis and date, and candidate set. Because COMET does not evaluate performance, we plan to couple it with performance-prediction models, to compare cost per unit of product, and to validate it against industrial prices under matched conditions.
