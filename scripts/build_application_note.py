...
COMET is a desktop application (Figure 1). A user enters a formulation, selects a preparation method and production scale, chooses a price basis, and obtains an estimate listing its sources, assumed inputs, and missing inputs; saved estimates can be repriced, compared, and ranked. A Python backend (FastAPI, SQLite) serves a React/TypeScript interface in English and Korean, packaged for Windows with Electron.
...
COMET implements the Step Method and records, with each estimate, its input sources, price date, and candidate set. It reproduces the three published examples line by line; the largest price difference, {largest_residual}%, arises from the margin rule. Production scale sets the estimated cost of a base-metal catalyst, metal loading that of a precious-metal catalyst, and treatment duration and recovered mass that of a laboratory batch, although only {n_mass} of {r(PREPARATION, 'profiles')} preparation records reports the recovered mass. Observed metal prices changed the lowest-cost candidate in {cs('summary.monthly.cost_winner')} of {cs('summary.monthly.families')} families, and recommendations changed with the candidate set and the price basis. Cost comparisons should therefore state production scale, assumed operating inputs, price basis and date, and candidate set. Because COMET does not evaluate performance, we plan to couple it with catalyst performance-prediction models, so that cost per unit of product can be compared, and to validate it against industrial prices under matched conditions.
