...
The note reuses the 2026-09-08 submission run, the joint robustness study and
...
Figure 2 summarises the cost model. Materials cost is the price-weighted mass of each component, or a purchased-input recipe when the precursor is specified; processing cost follows the Step Method, in which the hourly rates of the selected operations at the fitted scale class, escalated by the chemical producer price index, are multiplied by the campaign time and divided by the mass produced; general and administrative overhead, the sales and R&D uplift and the selling margin are applied as in the published method. Every ledger line carries the price used, its source, quote date and reliability grade, the price tier, and whether the operation was priced, substituted or left uncosted. For a 20 wt% Ni/Al₂O₃ catalyst prepared by incipient-wetness impregnation at a {ex('request.order_size_tons', '.0f')} t order on the live tier, the estimate is {ex('step_method.estimated_price_per_lb', '.2f')} USD/lb, of which nickel contributes {ex('materials.components[0].cost_per_lb_cat', '.2f')} USD/lb and processing {ex('step_method.processing_cost_per_lb', '.2f')} USD/lb (Figure 2b); the reference tier would price the same nickel at the frozen monthly average instead of the live quote (Figure 2c). Results export to CSV together with the price evidence and, when requested, the Monte Carlo range.

![Figure 2. Cost model and worked example. (a) Inputs, the three costing stages with their equations, and the ledger that carries the provenance of every value. (b) Build-up of the selling price for a 20 wt% Ni/Al₂O₃ catalyst prepared by incipient-wetness impregnation (six operations) at a 20 t order on the live price tier, quotes of 2026-09-10. (c) Monthly average prices of the platinum-group metals, gold, silver and nickel (Johnson Matthey and IMF PCPS, 2019-01 to 2026-07) with the frozen 2026-05 reference-tier value (teal) and the live quote held in the frozen price package (orange, observed 2026-09-04 to 2026-09-06); the shaded span is the 89-month window replayed in Figure 3. The example in b used the nickel quote of 2026-09-10. Symbols: w mass fraction; c price per lb; f, p, y retained fraction, purity and yield of a purchased input; H hourly rate; T campaign days; I index escalation; M mass produced; g, s, m the overhead, S&ARD and margin fractions.](figures-note-2026-09-09/fig2_cost_model.png)
...
Three kinds of diagnostic accompany a ranking. Weight sensitivity evaluates every family on a simplex grid of {r('s', 'weight_sensitivity.grid_points')} weight vectors and reports how often the balanced winner stays first (median {r('s', 'weight_sensitivity.median_balanced_winner_share_pct', '.2f')}% across families). A historical replay re-costs every candidate at each of {r('s', 'volatility.window.states')} monthly metal-price states from {r('s', 'volatility.window.first')} to {r('s', 'volatility.window.last')}, with support prices and source annotations held at the reference state, and identifies single-metal break-even prices. A seeded Monte Carlo range multiplies component prices and order size by uniform factors within declared bounds (by default 0.70 to 1.30 for active components, 0.80 to 1.20 for promoters, supports and order size, and 0.85 to 1.15 for electrode adjuncts); components in one role share a draw, failed trials are counted with their reasons, and an omitted seed yields independent samples.

The joint robustness study extends these screens to the full product of {rb('summary.months')} price states and {rb('summary.weight_points["0.05"]')} weight vectors at a 0.05 increment, or {rb('summary.joint_scenarios_all_families["0.05"]', ',')} family-month-weight scenarios (Figure 3a,b). It reports each candidate's first-rank share and its mean and worst regret in score points, deletes each nonwinning candidate with and without recomputing the economics range, and pushes author-assigned scores to the adverse corner of clipped boxes of 2, 5 and 10 points. Across the library the reference winner keeps first rank in a median {rb('summary.reference_winner_joint_share_median_pct', '.2f')}% of scenarios, candidate removal changes the winner in {rb('summary.candidate_removal_winner_changes')} of {rb('summary.candidate_removal_cases')} cases, and only {rb('summary.rubric_robust_family_counts["5"]')} families survive every 5-point score perturbation. These are properties of the library and its rubrics, not forecasts.
