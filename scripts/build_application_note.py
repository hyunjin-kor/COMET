...
Source code, the Windows installer and the portable archive are at https://github.com/hyunjin-kor/COMET under the PolyForm Noncommercial License 1.0.0, which permits noncommercial use; the license is not OSI-approved, and commercial licences are available on request from the corresponding author. The repository is archived under the concept DOI 10.5281/zenodo.21451931. The prepared version is {r('m', 'project_version')}. Windows users run the installer; on other systems the application is served to a browser from source with Python 3.11 or newer and Node.js 22 or newer, as described in the README, with the platform caveat stated above. The backend test suite runs with `python -m pytest backend/tests -q`, and the analyses in this note are regenerated with the commands recorded in the repository's paper index.
