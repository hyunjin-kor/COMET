...
Figure 1 shows the result screen. The headline price sits above a cost ledger in which each component row carries the price used, its source, quote year and reliability grade, and the preparation-step table lists hourly rates, scale substitutions and operations that were left uncosted. Results export to CSV together with the price evidence and, when requested, the Monte Carlo range.

![Figure 1. Result screen: the cost ledger with the price evidence behind every line and the manufacturing scope of the selected route.](../assets/screen-result.png)
...
The joint robustness study extends these screens to the full product of {rb('summary.months')} price states and {rb('summary.weight_points["0.05"]')} weight vectors at a 0.05 increment, or {rb('summary.joint_scenarios_all_families["0.05"]', ',')} family-month-weight scenarios (Figure 2). It reports each candidate's first-rank share and its mean and worst regret in score points, deletes each nonwinning candidate with and without recomputing the economics range, and pushes author-assigned scores to the adverse corner of clipped boxes of 2, 5 and 10 points. Across the library the reference winner keeps first rank in a median {rb('summary.reference_winner_joint_share_median_pct', '.2f')}% of scenarios, candidate removal changes the winner in {rb('summary.candidate_removal_winner_changes')} of {rb('summary.candidate_removal_cases')} cases, and only {rb('summary.rubric_robust_family_counts["5"]')} families survive every 5-point score perturbation. These are properties of the library and its rubrics, not forecasts.

![Figure 2. Joint retention of the balanced winner over historical price states and weight vectors (A) and the number of families affected by candidate removal or robust to score perturbations (B).]({ROBUSTNESS.rsplit('/', 1)[0]}/decision_robustness.png)
