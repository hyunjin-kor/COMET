...
def render():
    ...
    lines = ["# Supporting Information", "", "COMET: Catalyst Overall Manufacturing Estimation Tool", "",
             ...
             "This supplement documents the September 2026 COMET Application Note. The May 2026 screening results retain their original frozen formulations and assumptions. "
             ...
             "Batch purchases replace composition-based materials costs; batch operation costs replace Step Method processing costs. "
             "For each operation, electricity is measured kWh or the sum of mean power multiplied by ramp, hold and additional durations. "
             "A ramp takes absolute temperature change divided by the entered ramp rate. Equipment occupancy is charged for the full entered operation duration; "
             ...
             "For final dry mass M, manufacturing cost is (purchases + electricity + equipment + labor + gas + explicit additional charges)/M. "
             "Selling price applies sequential G&A and SARD factors, then divides by (1 − margin). The example excludes disposal, analytical testing, "
             ...
             "at 0.1 L/min and costs 10 USD/m3; both flow and price refer to 0 °C and 1 atm. Its chemical composition is unspecified because this is an arithmetic scenario.", "",
    ...
    lines += ["", "| Purchase at impregnation | Quantity | Unit price | Batch expenditure (USD) |", "|---|---:|---:|---:|"]
    ...
    lines += ["", "## S3. Independent arithmetic verification", "",
              "An independent balance uses explicit scalar arithmetic, without reading engine result fields. Thermal durations include ramp, hold and the additional hour. "
              "The full application API produces the same baseline and endpoint results. The regression also checks the application's seeded Monte Carlo summary and histogram counts.", "",
    ...
    lines += ["", "| Cost component | Incurred per batch (USD) | Per dry product (USD/kg) |", "|---|---:|---:|"]
    for key, label in [("materials_usd", "Purchases"), ("electricity_usd", "Electricity"), ("equipment_usd", "Equipment occupancy"),
        ...
        lines.append(f"| {label} | {hand[key]:.6f} | {hand[key]/.03:.6f} |")
    lines += ["", f"Manufacturing cost before overheads and margin is {hand['manufacturing_usd_kg']:.6f} USD/kg; "
              ...
              "Each endpoint changes only its named input; all other batch inputs remain fixed. Temperature does not determine an assumed power change. "
    ...
    lines += ["", "Figure 3(c) evaluates 21 calcination-hold values from 1 to 6 h at each of three dry masses (0.015, 0.030 and 0.045 kg), giving 63 scenarios. "
              "The complete grid is stored under `sweep` in manufacturing_study.json.", "",
              ...
              "These are scenario percentiles, not statistical confidence bounds. Individual sampled inputs and results are retained in the JSON. "
              "The application ordinarily excludes and counts invalid combinations without clamping; this example has none. "
              "Its random stream includes five fixed price/order multiplier columns before manufacturing draws, matching the API.", "",
              ...
              "An electrode candidate's powder cost is distinct from assembly cost per area. These observations do not establish equivalent activity or commercial quotation validity.", "",
    ...
    for family in data["screening"]["families"]:
        for c in family["candidates"]:
            lines.append(f"| {family['family']} | {cell(c['title'])} | {c['landed_cost_per_lb']*PER_LB_TO_PER_KG:.4f} | {c['lca']['coverage_pct']:.2f} |")
    lines += ["", "Ranking calculations use the original four criterion weights and assigned route/performance scores retained in the frozen methods and robustness files. "
              ...
              f"{sum(not c['profile_ids'] for c in library['candidates'])} have no curated preparation. Bibliographic verification covers {len(library['sources'])} DOIs. "
              ...
              "A source_mismatch flag is retained for discrepancies even where a related preparation is available.", "",
    ...
    for family in sorted({c["family"] for c in library["candidates"]}):
        ...
        lines.append(f"| {family} | {len(rows)} | {sum(bool(c['profile_ids']) for c in rows)} | {sum(c['status']=='source_mismatch' for c in rows)} |")
    statuses = Counter(c["status"] for c in library["candidates"])
    lines += ["", "Mutually exclusive catalog assessment counts: " + "; ".join(f"{key}: {value}" for key, value in sorted(statuses.items())) + ".", "",
              ...
              "Operating references preserve geography, period and basis. EIA electricity averages can be selected as explicit scenarios; "
              "BLS wage statistics and manufacturer connected-load ratings remain references, not measured batch costs or average operating power. "
              "Actual staffing, utility consumption and supplier prices require separate evidence.", "",
              "## S7. Reproduction and data files", "",
              "Run from the supplied COMET source root with its declared project dependencies. No paid data access is needed to regenerate the frozen analyses. "
              "Regeneration reads stored inputs; refreshing an external source is a separate operation. The software version label alone does not identify these local changes; "
              "the per-file checksums in the accompanying package identify the supplied source snapshot.", "",
              "```text", "python scripts/reproduce_manufacturing_study.py --check", "python scripts/build_manufacturing_literature_review.py --check",
              "python -m scripts.build_note_si --check", "python scripts/draw_application_note_figures.py", "python scripts/draw_application_note_figures.py --lang ko",
              "python scripts/build_application_note.py --check", "python -m pytest backend/tests/test_application_note.py -q", "python -m ruff check scripts/", "```", "",
              "The manufacturing regression is `backend/tests/test_manufacturing_study.py`. It compares independent arithmetic with the calculation, sensitivity and Monte Carlo APIs. "
              "Source fidelity, missing-input handling, persistence, transfers and comparisons have separate regression tests. "
              "Native PowerPoint source decks and export manifests identify conceptual-artwork labels; numerical panels come from frozen JSON. "
              "The TOC uses programmatically drawn artwork without generative images.", "",
              "| Input record | SHA-256 of supplied file |", "|---|---|"]
    for path in sources.values():
        lines.append(f"| {path} | {hashlib.sha256((ROOT/path).read_bytes()).hexdigest()} |")
    lines += ["", "Companion documents: `manufacturing-literature-2026-09-14.md` (complete preparation evidence) and "
              "`application-note-2026-09-09.md` (main article and reference list). Machine-readable JSON retains additional rows and original units; "
              "publication tables and plots use kg-based powder costs. Software is under PolyForm Noncommercial 1.0.0; third-party source terms remain applicable.", ""]
    return "\n".join(lines)
