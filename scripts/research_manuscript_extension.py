...
"""
    ...
    draft = draft[:abstract_start] + f"""Catalyst recommendations depend on prices, manufacturing assumptions and the evidence attached to their inputs. We present the Catalyst Overall Manufacturing Estimation Tool (COMET), an independently developed methodology and software workflow connecting traceable price states, explicit manufacturing boundaries, partial environmental inventories and reproducible catalyst decision analysis. Adopted thermal costing is checked against published reference cases without fitting material inputs. The library connects {r('summary.candidates')} candidates across {r('summary.families')} reaction families to traceable price states. Joint enumeration of {r('summary.months')} monthly metal states and {r('summary.weight_points["0.05"]')} preference vectors gives a median reference-winner retention of {r('summary.reference_winner_joint_share_median_pct', '.2f')}%. Removing a nonwinning candidate reverses the selected winner in {r('summary.candidate_removal_families_changed')} families through economic re-normalization. Only {r('summary.rubric_robust_family_counts["5"]')} families remain robust to all route/performance score perturbations within an analyst-selected ±5-point box. These are conditional model results, not future winning probabilities. Controlled price/evidence comparisons and fixed-composition manufacturing scenarios distinguish source annotations and discrete scale assumptions from measured economic advantage. Environmental results remain partial inventories, while electrode assemblies retain their area boundary where input completeness permits it. Public procurement evidence is unmatched for independent full-cost validation, so empirical predictive accuracy remains unestimated. COMET makes the dependence of catalyst choices on evidence, costs and preferences inspectable within one reproducible workflow.""" + draft[abstract_end:]
...
"""
    ...
    si = si.replace("## Table S1.", "COMET connects price evidence, manufacturing boundaries and decision diagnostics in an independently developed workflow. Published Step Method and CatCost rates and correlations remain adopted prior methods; source records below retain their actual origins. Tables S6–S8 document COMET's controlled and robustness analyses, with their stated model limits.\n\n## Table S1.", 1)
