...
Get the installer from the [latest release](https://github.com/hyunjin-kor/CatPrice/releases/latest):

- `CatPrice.Setup.<version>.exe` — recommended for most users
- `CatPrice-win-unpacked.zip` — portable version, runs without installation
...
The build produces `dist-electron\CatPrice Setup <version>.exe` and an unpacked app at `dist-electron\win-unpacked\CatPrice.exe`. Running instances are stopped automatically before a rebuild, or manually with `npm run desktop:stop`.
...
CatPrice runs without any keys. Add them only if you want live price feeds:
...
CatPrice is an independent implementation. It cites the CatCost methodology academically but does not redistribute CatCost source data, and it is not affiliated with or endorsed by NREL.
